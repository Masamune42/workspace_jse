...
public class CatalogueManager {
	...
	/**
	 * majArticle : Modifier ou ajouter un article à partir des données ihm
	 * 
	 * @throws BLLException
	 */
	public void majArticle(int indexCatalogue, Article articleAffiche) throws BLLException{

		Article a = null;
		try {
			//Préparation de l'objet pour l'Update
			if (indexCatalogue >= catalogue.size()) {
				if (articleAffiche instanceof Ramette) {
					a = new Ramette();
				} else {
					a = new Stylo();
				}
			} else 
				//Récupération de l'article en mémoire
				a = catalogue.get(indexCatalogue);
			//Mise à jour ou remplissage de l'objet
			if (articleAffiche instanceof Ramette) {
				((Ramette)a).setGrammage(((Ramette) articleAffiche).getGrammage());
			} else {
				((Stylo)a).setCouleur(((Stylo)articleAffiche).getCouleur());
			}
			a.setReference(articleAffiche.getReference());
			a.setMarque(articleAffiche.getMarque());
			a.setDesignation(articleAffiche.getDesignation());
			a.setPrixUnitaire(articleAffiche.getPrixUnitaire());
			a.setQteStock(articleAffiche.getQteStock());
	...
	/**
	 * updateArticle : Mettre à jour un article en BD
	 * @param article
	 * @throws BLLException
	 */
	public void updateArticle(Article article) throws BLLException{
			...
			try {
				...
				throw new BLLException("Echec updateArticle - ", e);
			}
